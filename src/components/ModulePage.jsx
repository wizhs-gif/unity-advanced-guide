...
export default function ModulePage({ moduleKey, progress, toggleTask, children }) {
  ...
  return (
    <div className="module-wrapper">
      <div className="module-task-panel">
        ...
        <div className="module-tasks">
          {tasks.map(task => (
            <label key={task.id} className={`module-task-item ${progress[task.id] ? 'done' : ''}`}>
              <input type="checkbox" checked={!!progress[task.id]} onChange={() => toggleTask(task.id)} />
              <span className="module-task-check" />
              <span className="module-task-text">{task.title}</span>
            </label>
          ))}
        </div>
        {isComplete && (
